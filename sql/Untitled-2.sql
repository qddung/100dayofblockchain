...
```sql
CREATE PROCEDURE sp_NhanVienChuaLapPhieuNhap
    @nam INT
AS
BEGIN
    SELECT 
...
```sql
CREATE PROCEDURE sp_ChiTietXuatHoaDon
    @sohd NCHAR(8)
AS
BEGIN
    SELECT 
...
```sql
CREATE PROCEDURE sp_PhieuNhap6ThangDau
    @nam INT
AS
BEGIN
    SELECT 
...
```sql
CREATE PROCEDURE sp_DemPhieuNhap
AS
BEGIN
...
```sql
CREATE PROCEDURE sp_NhanVienTheoLuong
    @luongmin FLOAT,
    @luongmax FLOAT
AS
BEGIN
...
```sql
CREATE PROCEDURE sp_PhieuTheoLoaiThoiGian
    @loai NVARCHAR(20),
    @tungay DATE,
    @denngay DATE
AS
...
```sql
CREATE PROCEDURE sp_DemLanNhapXuatVatTu
    @mavt NCHAR(4)
AS
BEGIN
...
```sql
CREATE PROCEDURE sp_ThongKePhieuTheoNhanVien
    @loai NVARCHAR(20),
    @nam INT
AS
BEGIN
...
```sql
CREATE PROCEDURE sp_TonTatCaVatTu
AS
BEGIN
    SELECT 
        V.MAVT,
...
```sql
CREATE PROCEDURE sp_PhieuChiTietNhanVien
    @loai NVARCHAR(20),
    @manv INT,
    @nam INT
AS
...
```sql
CREATE PROCEDURE sp_Tao_Phieu_Nhap
    @SoPN NCHAR(8),
    @NgayNhap DATE,
    @MaNV INT,
    @SoHieu NCHAR(8)
...
```sql
CREATE PROCEDURE sp_Cap_Nhat_Muc_Giam_Gia
